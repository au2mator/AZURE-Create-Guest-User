...
# Type: New Service
#
# Title: AZURE -Create a Guest User
#
# v 1.0 Initial Release
# 
#
# Init Release: 23.02.2022
# Last Update: 23.02.2022
# Code Template V 1.4
#
# Github: https://click.au2mator.com/AZURE_CreateGuestUser
#
# PreReq: Make sure ActiveDirectory PowerShell is able to install and run
# PreReq: au2mator 4.0 or higher required
# PreReq: create a Teams App Reg for Teams Cards, starting line 128
# See Varibales in "region CustomVariables"
#
#################
...
$AzureADGraphAPICred_File = "AzureADCreds.xml"
$AzureADGraphAPICred = Import-CliXml -Path (Get-ChildItem -Path $CredentialStorePath -Filter $AzureADGraphAPICred_File).FullName
$AzureADclientId = $AzureADGraphAPICred.clientId
$AzureADclientSecret = $AzureADGraphAPICred.clientSecret
$AzureADtenantName = $AzureADGraphAPICred.tenantName
...
$CCRecipient = Get-ADUser -identity $InitiatedBy.split("\")[1]
$InviteMessage = "You have been invited to join the Tenant au2mator.com"

try {
    Write-au2matorLog -Type INFO -Text "Try to connect to Azure AD Graph API"
    ...
    $tokenResponse = Invoke-RestMethod -Uri "https://login.microsoftonline.com/$AzureADtenantName/oauth2/v2.0/token" -Method POST -Body $tokenBody  
    ...
    try {
        Write-au2matorLog -Type INFO -Text "Try to send Invite to our Tenant"
        ...
        {
            "invitedUserEmailAddress":"$c_GuestMail",
            "inviteRedirectUrl":"https://au2mator.com",
            "invitedUserDisplayName":"$c_GuestName",
                ...
                "ccRecipients": [
                     {
                        "emailAddress": {
                            ...
                            "address": "$($CCRecipient.UserPrincipalName)"
                         }
                     }
                ],
...
"@
        ...
        Invoke-RestMethod -Method $Method -Uri $URL -Body $bodyNewVMexistingNetwork -Headers $headers


    }
    catch {
        Write-au2matorLog -Type ERROR -Text "Error to invite our Guest User"
        Write-au2matorLog -Type ERROR -Text $Error
    
        $au2matorReturn = "Error to invite our Guest User, Error: $Error"
        $TeamsReturn = "Error to invite our Guest User" #No Special Characters allowed
        $AdditionalHTML = "Error to invite our Guest User
        <br>
        Error: $Error
            "
        $Status = "ERROR"
    ...
    Write-au2matorLog -Type ERROR -Text "Failed to authenticate with Azure GRAPH API"
    Write-au2matorLog -Type ERROR -Text $Error

    $au2matorReturn = "Failed to authenticate with Azure GRAPH API, Error: $Error"
    $TeamsReturn = "Failed to authenticate with Azure GRAPH API" #No Special Characters allowed
    $AdditionalHTML = "Failed to authenticate with Azure GRAPH API
    <br>
    Error: $Error
        "
    $Status = "ERROR"
